...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	...
	*/
    ...
    // 2. Pintar la fila de captura (Si está en Borrador)
        ...
        print '<td>';
        // Hacemos un selectarray nativo alimentado por un Query manual a llx_product
        if (!is_object($form)) {
            require_once DOL_DOCUMENT_ROOT.'/core/class/html.form.class.php';
            $form = new Form($db);
        }
        ...
        // Jalamos los productos activos (tostock=1 o tosell=1 o tobuy=1, filtramos los que manejan stock)
        $sql_prod = "SELECT rowid, ref, label FROM ".MAIN_DB_PREFIX."product WHERE tosell=1 OR tobuy=1";
        $res_prod = $db->query($sql_prod);
        if ($res_prod) {
            while ($pobj = $db->fetch_object($res_prod)) {
                $lista_productos[$pobj->rowid] = '['.$pobj->ref.'] '.$pobj->label;
            }
        }
        
        // Pintamos el combo desplegable oficial con nuestra lista manual
        print $form->selectarray('idprod', $lista_productos, GETPOST('idprod', 'int'), 1, 0, 0, '', 1, 0, 0, '', 'minwidth300 chzn-select');
        print '</td>';
    ...
    print '</table>';
    print '</form>';
    print '<script type="text/javascript">jQuery(document).ready(function() { jQuery(".chzn-select").chosen(); });</script>';
