...
/**
 *   	\file       traspaso_list.php
 *		\ingroup    traspasomultiempresa
 *		\brief      List page for traspaso
 */
...
require_once __DIR__.'/class/traspaso.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('traspasomultiempresa', 'traspaso', 'write');
	$permissiontodelete = $user->hasRight('traspasomultiempresa', 'traspaso', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Traspaso';
	$objectlabel = 'Traspaso';
...
$title = $langs->trans("Traspasos");
//$help_url = "EN:Module_Traspaso|FR:Module_Traspaso_FR|ES:Módulo_Traspaso";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/traspasomultiempresa/traspaso_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$topicmail = "SendTraspasoRef";
$modelmail = "traspaso";
$objecttmp = new Traspaso($db);
