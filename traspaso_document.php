...
/**
 *  \file       traspaso_document.php
 *  \ingroup    traspasomultiempresa
 *  \brief      Tab for documents linked to Traspaso
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->traspasomultiempresa->multidir_output[$object->entity ? $object->entity : $conf->entity]."/traspaso/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('traspasomultiempresa', 'traspaso', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("Traspaso")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("Traspaso"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='traspaso/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'traspaso/'.dol_sanitizeFileName($object->ref).'/';
