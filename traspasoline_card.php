...
/**
 *    \file       traspasoline_card.php
 *    \ingroup    traspasomultiempresa
 *    \brief      Page to create/edit/view traspasoline
 */
...
dol_include_once('/traspasomultiempresa/lib/traspasomultiempresa_traspasoline.lib.php');
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('traspasomultiempresa', 'traspasoline', 'read');
	$permissiontoadd = $user->hasRight('traspasomultiempresa', 'traspasoline', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->hasRight('traspasomultiempresa', 'traspasoline', 'delete') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->hasRight('traspasomultiempresa', 'traspasoline', 'write'); // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->hasRight('traspasomultiempresa', 'traspasoline', 'write'); // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->traspasomultiempresa->multidir_output[isset($object->entity) ? $object->entity : 1].'/traspasoline';
...
if (empty($reshook)) {
	$backurlforlist = dol_buildpath('/traspasomultiempresa/traspasoline_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/traspasomultiempresa/traspasoline_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
	...
	$trackid = 'traspasoline'.$object->id;
...
$title = $langs->trans("Traspasoline")." - ".$langs->trans('Card');
//$title = $object->ref." - ".$langs->trans('Card');
if ($action == 'create') {
	$title = $langs->trans("NewObject", $langs->transnoentitiesnoconv("Traspasoline"));
}
$help_url = '';
...
if (($id || $ref) && $action == 'edit') {
	print load_fiche_titre($langs->trans("Traspasoline"), '', $object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	$head = traspasolinePrepareHead($object);

	print dol_get_fiche_head($head, 'card', $langs->trans("Traspasoline"), -1, $object->picto, 0, '', '', 0, '', 1);

	$formconfirm = '';

	// Confirmation to delete (using preloaded confirm popup)
	if ($action == 'delete' || ($conf->use_javascript_ajax && empty($conf->dol_use_jmobile))) {
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteTraspasoline'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 'action-delete');
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/traspasomultiempresa/traspasoline_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	*/
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			print $formfile->showdocuments('traspasomultiempresa:Traspasoline', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$tmparray = $form->showLinkToObjectBlock($object, array(), array('traspasoline'), 1);
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/traspasomultiempresa/traspasoline_agenda.php', 1).'?id='.$object->id);
