...
/**
 *	\file       htdocs/core/modules/traspasomultiempresa/doc/doc_generic_traspaso_odt.modules.php
 *	\ingroup    traspasomultiempresa
 *	\brief      File of class to build ODT documents for traspasos
 */
 ...
 */
class doc_generic_traspaso_odt extends ModelePDFTraspaso
{
	/**
	 * Issuer
	 * @var Societe
	...
	public function __construct($db)
	{
		...
		$this->scandir = 'TRASPASOMULTIEMPRESA_MYOBJECT_ADDON_PDF_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function info($langs)
	{
		...
		$odtPath = trim(getDolGlobalString('TRASPASOMULTIEMPRESA_MYOBJECT_ADDON_PDF_ODT_PATH'));
		...
		$texte .= '<input type="hidden" name="param1" value="TRASPASOMULTIEMPRESA_MYOBJECT_ADDON_PDF_ODT_PATH">';
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=traspasomultiempresa_traspaso/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=doctemplates&keyforuploaddir=TRASPASOMULTIEMPRESA_MYOBJECT_ADDON_PDF_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picto('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
		...
		if (!getDolGlobalString('MAIN_NO_MULTIDIR_FOR_ODT')) {
			...
			$texte .= '<br>';
			$texte .= '<input type="hidden" name="value1" value="TRASPASOMULTIEMPRESA_MYOBJECT_ADDON_PDF_ODT_PATH">';
		}
		...
		$texte .= '<input type="hidden" value="TRASPASOMULTIEMPRESA_MYOBJECT_ADDON_PDF_ODT_PATH" name="keyforuploaddir">';
	 ...
	 *
	 *  @param	Traspaso	$object				Source object to generate document from
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->traspasomultiempresa->dir_output) {
			// If $object is id instead of object
			if (!is_object($object)) {
				$id = $object;
				$object = new Traspaso($this->db);
			...
			$dir = $conf->traspasomultiempresa->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($conf->traspasomultiempresa->dir_temp);
				if (!is_writable($conf->traspasomultiempresa->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $conf->traspasomultiempresa->dir_temp);
				...
				// Line of free text
				$newfreetext = '';
				$paramfreetext = 'TRASPASOMULTIEMPRESA_MYOBJECT_FREE_TEXT';
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $conf->traspasomultiempresa->dir_temp,
