...
/**
 * \file        class/traspaso.class.php
 * \ingroup     traspasomultiempresa
 * \brief       This file is a CRUD class file for Traspaso (Create/Read/Update/Delete)
 */
 ...
 * Class for Traspaso
 */
class Traspaso extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'traspaso';

	/**
	 * @var string 		Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'traspasomultiempresa_traspaso';

	/**
	 * @var string 		If permission must be checkec with hasRight('traspasomultiempresa', 'read') and not hasright('mymodyle', 'traspaso', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'traspasomultiempresa';

	/**
	 * @var string 		String with name of icon for traspaso. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'traspaso@traspasomultiempresa' if picto is file 'img/object_traspaso.png'.
	 */
	...
	public $fields = array(
		...
		"fk_user_modif" => array("type" => "integer:User:user/class/user.class.php", "label" => "Usuario Validador", "picto" => "user", "enabled" => "1", 'position' => 511, 'notnull' => -1, "visible" => "1", "csslist" => "tdoverflowmax150",),
	...
	public $fk_warehouse_origen;
	public $fk_warehouse_destino;
	...
	//  */
	public $table_element_line = 'traspasomultiempresa_traspasoline';
	...
	//  */
	public $fk_element = 'fk_traspaso';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	public $class_element_line = 'Traspasoline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Traspaso', 'fk_element'=>'fk_traspaso'));
	...
	//  */
	// protected $childtablesoncascade = array('traspasomultiempresa_traspasodet');

	// /**
	//  * @var TraspasoLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('traspasomultiempresa', 'traspaso', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('traspasomultiempresa', 'traspaso', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('traspasomultiempresa', 'traspaso_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'traspaso/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'traspaso/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'traspaso/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'traspaso/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->traspasomultiempresa->dir_output.'/traspaso/'.$oldref;
				$dirdest = $conf->traspasomultiempresa->dir_output.'/traspaso/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->traspasomultiempresa->dir_output.'/traspaso/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowTraspaso")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Traspaso").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/traspasomultiempresa/traspaso_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowTraspaso");
	...
	public function LibStatut($status, $mode = 0)
	{
		...
		if (empty($this->labelStatus) || empty($this->labelStatusShort)) {
			...
			$this->labelStatus[self::STATUS_VALIDATED] = 'Validado';
			$this->labelStatus[self::STATUS_CANCELED] = $langs->transnoentitiesnoconv('Disabled');
			$this->labelStatusShort[self::STATUS_DRAFT] = $langs->transnoentitiesnoconv('Draft');
			//$this->labelStatusShort[self::STATUS_VALIDATED] = $langs->transnoentitiesnoconv('Enabled');
			$this->labelStatusShort[self::STATUS_VALIDATED] = 'Validado';
	...
	public function getLinesArray()
	{
		...
		$objectline = new Traspasoline($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_traspaso:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('TRASPASOMULTIEMPRESA_MYOBJECT_ADDON')) {
			$conf->global->TRASPASOMULTIEMPRESA_MYOBJECT_ADDON = 'mod_traspaso_standard';
		}
		...
		if (getDolGlobalString('TRASPASOMULTIEMPRESA_MYOBJECT_ADDON')) {
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefTraspaso $obj';
