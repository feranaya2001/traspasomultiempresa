...
/**
 * \file        class/traspasoline.class.php
 * \ingroup     traspasomultiempresa
 * \brief       This file is a CRUD class file for Traspasoline (Create/Read/Update/Delete)
 */
...
/**
 * Class for Traspasoline
 */
if (!class_exists('TraspasoLine')) {

class TraspasoLine extends CommonObject {
	/**
	 * @var string 		ID of module.
	 */
	public $module = 'traspasomultiempresa';

	/**
	 * @var string 		ID to identify managed object.
	 */
	public $element = 'traspasoLline';

	/**
	 * @var string 		Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'traspasomultiempresa_traspasoline';

	/**
	 * @var string 		If permission must be checkec with hasRight('traspasomultiempresa', 'read') and not hasright('mymodyle', 'traspasoline', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'traspasomultiempresa';

	/**
	 * @var string 		String with name of icon for traspasoline. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'traspasoline@traspasomultiempresa' if picto is file 'img/object_traspasoline.png'.
	 */
	...
	public $fields = array(
		...
		//"fk_traspaso" => array("type" => "integer", "label" => "Traspaso Padre", "enabled" => "1", 'position' => 0, 'notnull' => 1, "visible" => "1", "foreignkey" => "0",),
		"fk_traspaso" => array("type" => "integer", "label" => "Traspaso Padre", "foreignkey" => "traspaso:rowid", "enabled" => "1", 'position' => 0, 'notnull' => 1, "visible" => "1",),
		//"fk_product" => array("type" => "sellist", "label" => "Producto", "enabled" => "1", 'position' => 2, 'notnull' => 1, "visible" => "1", "foreignkey" => "0", "arrayofkeyval" => array("0" => "product:ref:rowid::tosell=1"),),
		"fk_product" => array("type" => "sellist", "label" => "Producto", "foreignkey" => "product:rowid", "enabled" => "1", 'position' => 2, 'notnull' => 1, "visible" => "1", "arrayofkeyval" => array("0" => "product:ref:rowid::tosell=1"),),
	);
	public $rowid;
	public $ref;
	public $label;
	...
	//  */
	// public $table_element_line = 'traspasomultiempresa_traspasolineline';
	...
	//  */
	// public $fk_element = 'fk_traspasoline';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Traspasolineline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Traspasoline', 'fk_element'=>'fk_traspasoline'));
	...
	//  */
	// protected $childtablesoncascade = array('traspasomultiempresa_traspasolinedet');

	// /**
	//  * @var TraspasolineLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('traspasomultiempresa', 'traspasoline', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('traspasomultiempresa', 'traspasoline', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('traspasomultiempresa', 'traspasoline_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'traspasoline/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'traspasoline/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'traspasoline/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'traspasoline/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->traspasomultiempresa->dir_output.'/traspasoline/'.$oldref;
				$dirdest = $conf->traspasomultiempresa->dir_output.'/traspasoline/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->traspasomultiempresa->dir_output.'/traspasoline/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('traspasomultiempresa','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('traspasomultiempresa','traspasomultiempresa_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'TRASPASOMULTIEMPRESA_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'TRASPASOMULTIEMPRESA_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'TRASPASOMULTIEMPRESA_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowTraspasoline")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Traspasoline").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/traspasomultiempresa/traspasoline_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowTraspasoline");
	...
	public function getLinesArray()
	{
		...
		$objectline = new TraspasolineLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_traspasoline:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('TRASPASOMULTIEMPRESA_MYOBJECT_ADDON')) {
			$conf->global->TRASPASOMULTIEMPRESA_MYOBJECT_ADDON = 'mod_traspasoline_standard';
		}

		if (getDolGlobalString('TRASPASOMULTIEMPRESA_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('TRASPASOMULTIEMPRESA_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('TRASPASOMULTIEMPRESA_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/traspasomultiempresa/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefTraspasoline $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/traspasomultiempresa/doc/";
...
/**
 * Class TraspasolineLine. You can also remove this and generate a CRUD class for lines objects.
 */
class TraspasolineLine extends CommonObjectLine
{
	// To complete with content of an object TraspasolineLine
	// We should have a field rowid, fk_traspasoline and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'traspasoline'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_traspasoline'
