...
/**
 *   	\file       traspasoline_list.php
 *		\ingroup    traspasomultiempresa
 *		\brief      List page for traspasoline
 */
...
require_once __DIR__.'/class/traspasoline.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('traspasomultiempresa', 'traspasoline', 'write');
	$permissiontodelete = $user->hasRight('traspasomultiempresa', 'traspasoline', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("traspasomultiempresa")) {
	accessforbidden('Module traspasomultiempresa not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Traspasoline';
	$objectlabel = 'Traspasoline';
	$uploaddir = $conf->traspasomultiempresa->dir_output;
...
$title = $langs->trans("Traspasolines");
//$help_url = "EN:Module_Traspasoline|FR:Module_Traspasoline_FR|ES:Módulo_Traspasoline";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/traspasomultiempresa/traspasoline_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-traspasomultiempresa page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/traspasomultiempresa/traspasoline_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendTraspasolineRef";
$modelmail = "traspasoline";
$objecttmp = new Traspasoline($db);
